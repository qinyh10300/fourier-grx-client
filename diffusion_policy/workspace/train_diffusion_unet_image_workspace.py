...
from diffusion_policy.workspace.utils import param_groups_lrd
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # compute loss
                        raw_loss_dict = self.model(batch)
                        loss = raw_loss_dict['loss'] / cfg.training.gradient_accumulate_every
                        loss.backward()
                        ...
                        # logging
                        raw_bc_loss_cpu = raw_loss_dict['bc_loss'].item()
                        raw_repr_loss_cpu = raw_loss_dict['repr_loss'].item()
                        tepoch.set_postfix(loss=raw_bc_loss_cpu, refresh=False)
                        train_losses.append(raw_bc_loss_cpu)
                        step_log = {
                            'train_loss': raw_bc_loss_cpu,
                            'train_repr_loss': raw_repr_loss_cpu,
