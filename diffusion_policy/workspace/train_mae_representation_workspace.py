...
from omegaconf import OmegaConf
from diffusion_policy.model.tactile.mae import *
import numpy as np
import random
...
class TrainMaeRepresentationWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        for e in range(self.total_epoch):
            model.train()
            losses = []
            for obs in tqdm(iter(dataloader)):
                tactile_img = obs['tactile'].permute(0, 3, 1, 2).to(device)
                predicted_img, mask = model(tactile_img)
                loss = torch.mean((predicted_img - tactile_img) ** 2 * mask) / self.mask_ratio
                loss.backward()
                optim.step()
                optim.zero_grad()
