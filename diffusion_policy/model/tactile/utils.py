...
class ConvPoolingHead(nn.Module):
    def __init__(self, input_channels, conv1_out_channels=128, conv2_out_channels=256, conv3_out_channels=512,
        ...
        self.conv1 = nn.Conv2d(in_channels=input_channels, out_channels=conv1_out_channels, kernel_size=kernel_size,
                               padding=padding)
        ...
        self.conv2 = nn.Conv2d(in_channels=conv1_out_channels, out_channels=conv2_out_channels, kernel_size=kernel_size,
                               padding=padding)
        self.gn2 = nn.GroupNorm(group_num, conv2_out_channels)
        self.se2 = SEBlock(conv2_out_channels)
        self.conv3 = nn.Conv2d(in_channels=conv2_out_channels, out_channels=conv3_out_channels, kernel_size=kernel_size,
                               padding=padding)
...
class ResNetPerception(nn.Module):
    def __init__(self, num_classes=4, hidden1_dim=256, hidden2_dim=128, dropout_rate=0.2, backbone='resnet34',
        ...
        self.model.fc = MlpHead(in_features, hidden1_dim=hidden1_dim, hidden2_dim=hidden2_dim, output_dim=num_classes,
